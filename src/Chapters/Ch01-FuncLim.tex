...
$\theta=\frac{\pi}4$时，$\cos\theta=\sin\theta$，
...
\begin{shaded}
	\pss{\centering
		...
		{./images/ch01/Jakob_Bernoulli.jpg}\\
		\href{https://en.wikipedia.org/wiki/Jacob_Bernoulli}
		{Jacob Bernoulli (1655-1705)}\\
		伟大的Bernoulli家族第一代的杰出代表\\[1cm]
		\includegraphics[width=0.75\marginparwidth]
		{./images/ch01/Leonhard_Euler.jpg}\\
		\href{https://en.wikipedia.org/wiki/Leonhard_Euler}
		{Leonhard Euler (1707-1783)}\\
		历史上最高产的数学家之一
	}
	{\bf \href{https://en.wikipedia.org/wiki/E_(mathematical_constant)}
	{常数$e$}}
...
\pss{\centering
	\includegraphics[width=\marginparwidth]{./Images/Ch01/enn1.pdf}\\
	数列$\left\{\left(1+\frac1n\right)^n\right\}$和数列
	$\left\{\left(1+\frac1n\right)^{n+1}\right\}$
}
...
%{\bf 思考：} 设$\limx0\df{a\tan x+b(1-\cos x)}
%{c\ln(1-2x)+d(1-e^{-x^2})}=2$，已知$ac\ne 0$，证明：$a=-4c$。
...
% 	\item $\limx0\df1{x^3}\left[\left(\df{2+\cos }3\right)^x
% 	-1\right]$
% 	\item $\limn\ln\left(\df{n-2na+1}{n(1-2a)}\right)^n
% 	\quad(a\ne1/2)$
% 	\item $\limx0\df{[\sin x-\sin(\sin x)]\sin x}{x^4}$
% 	\item $\limx0\df{e^{\tan x}-e^{\sin x}}{x\sin^2x}$
% 	\item $\limx0\df{\sqrt{1+\tan x}-\sqrt{1+\sin x}}
% 	{x(1-\cos x)}$
% 	\item $\limn\left(n\tan\df1n\right)^{n^2}$
% 	\item $\limx0\df{\sin x+x^2\sin\df1x}{(1+\cos x)\ln(1+x)}$
% 	\item $\limx0\left[\df
% 	  ax-\left(\df1{x^2}-a^2\right)\ln(1+ax)\right]$
% 	  \quad($a\ne 0$)
% 	\item $\limx{0}\left(\df{a^{x+1}+b^{x+1}+c^{x+1}}{a+b+c}\right)
% 	^{1/x}
% 	  \,(a,b,c>0)$
% 	\item $\limx{0}\df{\tan(\tan x)-\sin(\sin x)}{\tan x-\sin x}$ 
% 	\item $\limx{\infty}\df{(x+a)^{x+a}(x+b)^{x+b}}
% 	{(x+a+b)^{2x+a+b}}$
...
% 	\item $\limn n^2\left(\arctan\df an
% 	-\arctan\df a{n+1}\right)\quad a>0)$
% 	\item $\limx0\df{(1+x)^{\frac1x}-(1+2x)^{\frac1{2x}}}{\sin x}$
% 	%\item $\limn\left[\left(n^3-n^2+\df n2\right)
% 	%^{\frac1n}-\sqrt{1+n^6}\right]$
%   \end{enumerate}
